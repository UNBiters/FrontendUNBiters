...
export default function MyComments({ comment, deleteCommentUp, editCommentUp }) {
    ...
    return (
        <>
            ...
            <div className="mt-2 bg-white relative  mx-4  flex justify-between gap-x-6 p-5 rounded-md">
                <div className=" flex min-w-0 gap-x-4">
                    <img className="h-12 w-12 flex-none rounded-full bg-gray-50" 
                    
                    src="/images/default.png" alt="" />
                    <div className="min-w-0 flex-auto">
                        <p className="text-sm font-semibold leading-6 text-gray-900">{comment.user.nombre}</p>
                        <p className="mt-1 text-clip  text-xs leading-5 text-gray-500">{comment.review}</p>
                    </div>
                    <button
                    ...
                    <button
                        className="!absolute top-4 right-12 h-8 max-h-[32px] w-8 max-w-[32px] select-none rounded-full text-center align-middle font-sans text-xs font-medium uppercase text-red-500 transition-all hover:bg-red-500/10 active:bg-red-500/30 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                        type="button"
                ...
                <div className="pt-10 hidden sm:flex sm:flex-col sm:items-end">
                    {/*<button className="flex text-sm leading-6 text-gray-900">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
